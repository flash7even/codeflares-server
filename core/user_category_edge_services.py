...
def print_user_root_synced_data(user_id):
    app.logger.info('print_user_root_synced_data: ' + str(user_id))
    rs = requests.session()
    must = [
    ...
    app.logger.info('print_user_root_synced_data query_json: ' + json.dumps(query_json))
    search_url = 'http://{}/{}/{}/_search'.format(app.config['ES_HOST'], _es_index_user_category, _es_type)
    response = rs.post(url=search_url, json=query_json, headers=_http_headers).json()
    app.logger.info('print_user_root_synced_data response: ' + str(response))


def get_user_root_synced_data_by_id(data_id):
    app.logger.info('get_user_root_synced_data_by_id: ' + str(data_id))
    rs = requests.session()
    search_url = 'http://{}/{}/{}/{}'.format(app.config['ES_HOST'], _es_index_user_category, _es_type, data_id)
    response = rs.get(url=search_url, headers=_http_headers).json()
    app.logger.info('get_user_root_synced_data_by_id response: ' + str(response))
...
def add_user_category_data(user_id, category_id, data):
    try:
        ...
        if edge is None:
            ...
            app.logger.debug(f'After Inserting category synced data: {json.dumps(response)}')
